...
// SetCap set capacity
func (p *Pool) SetCap(capacity uint64) uint64 {
	p.capacity = capacity
	return p.capacity
}
...
// GetRWaitingWorkers get waiting workers
func (p *Pool) GetRWaitingWorkers() uint64 {
	size := uint64(len(p.chTask))
	return atomic.LoadUint64(&size)
}

// GetCompileTaskCount get ran workers
func (p *Pool) GetCompileTaskCount() uint64 {
	return atomic.LoadUint64(&p.compileTaskCount)
}
